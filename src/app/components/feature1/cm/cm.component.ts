...
export class CMComponent implements OnInit {
  ...
  split() {
    let url: string = `#/ABC/products/category/12345/details`
    let strArray = url.split('/') //split url based on /
    strArray.shift() // remove first element
    strArray.pop() // remove first element
    console.log("strArray", strArray);
    let str = strArray.join('/',)  //join str array with /
    console.log('str', str);
